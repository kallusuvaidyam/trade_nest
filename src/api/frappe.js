...
function redirectToLogin(forceReload = false) {
  const target = forceReload
    ? `/shop?logout=${Date.now()}#/login`
    : '/shop#/login'
...
export const api = {
  ...
  signup: (data) => request('POST', '/method/trade_nest.api.signup_customer', data),
  ...
  getItems: (page = 1, search = '', pageSize = 12, category = '', vendor = '') =>
    request('GET', `/method/trade_nest.api.get_items?page=${page}&search=${encodeURIComponent(search)}&page_length=${pageSize}&category=${encodeURIComponent(category)}&vendor=${encodeURIComponent(vendor)}`),
  getItem: (item_code, vendor = '') =>
    request('GET', `/method/trade_nest.api.get_item?item_code=${encodeURIComponent(item_code)}&vendor=${encodeURIComponent(vendor)}`),
  ...
  // Reviews
  submitReview: (data) =>
    request('POST', '/method/trade_nest.api.submit_review', data),
  getReviews: (item_code, page = 1) =>
    request('GET', `/method/trade_nest.api.get_reviews?item_code=${encodeURIComponent(item_code)}&page=${page}`),

  // Addresses
  getMyAddresses: () =>
    request('GET', '/method/trade_nest.api.get_my_addresses'),
  saveAddress: (data) =>
    request('POST', '/method/trade_nest.api.save_address', data),
  deleteAddress: (name) =>
    request('POST', '/method/trade_nest.api.delete_address', { name }),

  // Vendor registration (customer → vendor)
  registerAsVendor: (data) =>
    request('POST', '/method/trade_nest.api.register_as_vendor', data),
